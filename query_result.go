...
type QueryResult struct {
	cQueryResult C.ryu_query_result
...
func (queryResult *QueryResult) ToString() string {
	cString := C.ryu_query_result_to_string(&queryResult.cQueryResult)
...
func (queryResult *QueryResult) Close() {
	...
	C.ryu_query_result_destroy(&queryResult.cQueryResult)
...
func (queryResult *QueryResult) ResetIterator() {
	C.ryu_query_result_reset_iterator(&queryResult.cQueryResult)
}
...
func (queryResult *QueryResult) GetColumnNames() []string {
	...
	numColumns := int64(C.ryu_query_result_get_num_columns(&queryResult.cQueryResult))
	columns := make([]string, 0, numColumns)
	for i := int64(0); i < numColumns; i++ {
		var outColumn *C.char
		C.ryu_query_result_get_column_name(&queryResult.cQueryResult, C.uint64_t(i), &outColumn)
		defer C.ryu_destroy_string(outColumn)
...
func (queryResult *QueryResult) GetNumberOfColumns() uint64 {
	return uint64(C.ryu_query_result_get_num_columns(&queryResult.cQueryResult))
}
...
func (queryResult *QueryResult) GetNumberOfRows() uint64 {
	...
	return uint64(C.ryu_query_result_get_num_tuples(&queryResult.cQueryResult))
}

// HasNext returns true if there is at least one more tuple in the result set.
func (queryResult *QueryResult) HasNext() bool {
	return bool(C.ryu_query_result_has_next(&queryResult.cQueryResult))
}
...
func (queryResult *QueryResult) Next() (*FlatTuple, error) {
	...
	status := C.ryu_query_result_get_next(&queryResult.cQueryResult, &tuple.cFlatTuple)
...
func (queryResult *QueryResult) HasNextQueryResult() bool {
	return bool(C.ryu_query_result_has_next_query_result(&queryResult.cQueryResult))
}
...
func (queryResult *QueryResult) NextQueryResult() (*QueryResult, error) {
	...
	status := C.ryu_query_result_get_next_query_result(&queryResult.cQueryResult, &nextQueryResult.cQueryResult)
	if status != C.RyuSuccess {
...
func (queryResult *QueryResult) GetCompilingTime() float64 {
	var cQuerySummary C.ryu_query_summary
	C.ryu_query_result_get_query_summary(&queryResult.cQueryResult, &cQuerySummary)
	defer C.ryu_query_summary_destroy(&cQuerySummary)
	return float64(C.ryu_query_summary_get_compiling_time(&cQuerySummary))
}
...
func (queryResult *QueryResult) GetExecutionTime() float64 {
	...
	C.ryu_query_result_get_query_summary(&queryResult.cQueryResult, &cQuerySummary)
	defer C.ryu_query_summary_destroy(&cQuerySummary)
	return float64(C.ryu_query_summary_get_execution_time(&cQuerySummary))
}
