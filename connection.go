...
// Connection represents a connection to a Ryu database.
type Connection struct {
	cConnection C.ryu_connection
	database    *Database
	isClosed    bool
}
...
func OpenConnection(database *Database) (*Connection, error) {
	...
	status := C.ryu_connection_init(&database.cDatabase, &conn.cConnection)
...
func (conn *Connection) Close() {
	...
	C.ryu_connection_destroy(&conn.cConnection)
	conn.isClosed = true
}
...
func (conn *Connection) GetMaxNumThreads() uint64 {
	...
	C.ryu_connection_get_max_num_thread_for_exec(&conn.cConnection, &numThreads)
...
func (conn *Connection) SetMaxNumThreads(numThreads uint64) {
	C.ryu_connection_set_max_num_thread_for_exec(&conn.cConnection, C.uint64_t(numThreads))
}

// Interrupt interrupts the execution of the current query on the connection.
func (conn *Connection) Interrupt() {
	C.ryu_connection_interrupt(&conn.cConnection)
}
...
func (conn *Connection) SetTimeout(timeout uint64) {
	C.ryu_connection_set_query_timeout(&conn.cConnection, C.uint64_t(timeout))
}
...
func (conn *Connection) Query(query string) (*QueryResult, error) {
	...
	status := C.ryu_connection_query(&conn.cConnection, cQuery, &queryResult.cQueryResult)
	if status != C.RyuSuccess || !C.ryu_query_result_is_success(&queryResult.cQueryResult) {
		cErrMsg := C.ryu_query_result_get_error_message(&queryResult.cQueryResult)
		defer C.ryu_destroy_string(cErrMsg)
...
func (conn *Connection) Execute(preparedStatement *PreparedStatement, args map[string]any) (*QueryResult, error) {
	...
	status := C.ryu_connection_execute(&conn.cConnection, &preparedStatement.cPreparedStatement, &queryResult.cQueryResult)
	if status != C.RyuSuccess || !C.ryu_query_result_is_success(&queryResult.cQueryResult) {
		cErrMsg := C.ryu_query_result_get_error_message(&queryResult.cQueryResult)
		defer C.ryu_destroy_string(cErrMsg)
		return queryResult, fmt.Errorf(C.GoString(cErrMsg))
...
func (conn *Connection) bindParameter(preparedStatement *PreparedStatement, key string, value any) error {
	...
	var status C.ryu_state
	var cValue *C.ryu_value
	var valueConversionError error
	cValue, valueConversionError = goValueToRyuValue(value)
	if valueConversionError != nil {
		return fmt.Errorf("failed to convert Go value to Ryu value: %v", valueConversionError)
	}
	defer C.ryu_value_destroy(cValue)
	status = C.ryu_prepared_statement_bind_value(&preparedStatement.cPreparedStatement, cKey, cValue)
	if status != C.RyuSuccess {
...
func (conn *Connection) Prepare(query string) (*PreparedStatement, error) {
	...
	status := C.ryu_connection_prepare(&conn.cConnection, cQuery, &preparedStatement.cPreparedStatement)
	if status != C.RyuSuccess || !C.ryu_prepared_statement_is_success(&preparedStatement.cPreparedStatement) {
		cErrMsg := C.ryu_prepared_statement_get_error_message(&preparedStatement.cPreparedStatement)
		defer C.ryu_destroy_string(cErrMsg)
